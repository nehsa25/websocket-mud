...
class WorldEvents:
    class Weather:
        ...
        def set_weather_descriptions(self):
            method_name = inspect.currentframe().f_code.co_name
            LogUtils.debug(f"{method_name}: enter", self.logger)
            if self.weather_type == Utility.Share.WeatherTypes.RAIN and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "There's currently a light drizzle."
                self.start_description = "It's starting to rain."
            elif self.weather_type == Utility.Share.WeatherTypes.RAIN and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
                self.room_description = "It's raining."
                self.start_description = "It's starting to rain hard."
            elif self.weather_type == Utility.Share.WeatherTypes.RAIN and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
                self.room_description = "It's pouring rain."
                self.start_description = "It's starting to rain very hard."
            elif self.weather_type == Utility.Share.WeatherTypes.FOG and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "A small fog has settled in."
                self.start_description = "A light fog begins to form."
            elif self.weather_type == Utility.Share.WeatherTypes.FOG and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
                self.room_description = "Fog has settled in."
                self.start_description = "A mild fog begins to form."
            elif self.weather_type == Utility.Share.WeatherTypes.FOG and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
            ...
            elif self.weather_type == Utility.Share.WeatherTypes.SNOW and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "Small flurry of snowflakes are falling."
                self.start_description = "It begins to snow."
            elif self.weather_type == Utility.Share.WeatherTypes.SNOW and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
                self.room_description = "It's snowing."
                self.start_description = "A decent snowfall abruptly begins."
            elif self.weather_type == Utility.Share.WeatherTypes.SNOW and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
                self.room_description = "It's snowing heavily. You may want to find shelter."
                self.start_description = "A heavy snowfall begins."
            elif self.weather_type == Utility.Share.WeatherTypes.THUNDER and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "It's pouring rain."
                self.start_description = "It's starting to rain very hard. The sky is beginning to darken."
            elif self.weather_type == Utility.Share.WeatherTypes.THUNDER and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
            ...
            elif self.weather_type == Utility.Share.WeatherTypes.THUNDER and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
            ...
            elif self.weather_type == Utility.Share.WeatherTypes.SUN and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "The sun is shines down. It's pleasant."
                self.start_description = "Sun breaks through the clouds."
            elif self.weather_type == Utility.Share.WeatherTypes.SUN and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
                self.room_description = "The sun is shining brightly. You shade your eyes as you look around."
                self.start_description = "Sun breaks through the clouds."
            elif self.weather_type == Utility.Share.WeatherTypes.SUN and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
                self.room_description = "A cloudless blue sky extends in all directions."
                self.start_description = "Sun breaks through the clouds."
            elif self.weather_type == Utility.Share.WeatherTypes.OVERCAST and self.weather_strength == Utility.Share.WeatherStrength.LIGHT:
                self.room_description = "Small patches of sun shine through the clouds."
                self.start_description = "Clouds are starting to form."
            elif self.weather_type == Utility.Share.WeatherTypes.OVERCAST and self.weather_strength == Utility.Share.WeatherStrength.MEDIUM:
                self.room_description = "The sky is overcast."
                self.start_description = "Clouds are starting to form."
            elif self.weather_type == Utility.Share.WeatherTypes.OVERCAST and self.weather_strength == Utility.Share.WeatherStrength.HEAVY:
    ...
    def __init__(self, logger):
        method_name = inspect.currentframe().f_code.co_name
        self.logger = logger
        LogUtils.debug(f"{method_name}: Initializing WorldEvents class", self.logger)
        
        if self.weather is None:
            self.weather = WorldEvents.Weather(Utility.Share.WeatherTypes.FOG, 
                                               Utility.Share.WeatherStrength.MEDIUM, self.logger)
    ...
    async def check_for_combat(self):
        ...
        while not self.shutdown:
            ...
            try:                
                ...
                new_battle = await self.battles.check_for_combat(self.players, self.monsters, self.rooms)
                ...
                for current_battle in self.battles.active_battles:
                    current_battle, self = await self.battles.run_combat_round(current_battle, self.players, world=self)
                    if current_battle is not None and current_battle.state == current_battle.BattleState.COMPLETED:
    ...
    async def check_day_or_night(self):
        ...
        while not self.shutdown:
            try:
                ...
                self.dayornight = Utility.Share.DayOrNight.DAY if self.dayornight == Utility.Share.DayOrNight.NIGHT else Utility.Share.DayOrNight.NIGHT

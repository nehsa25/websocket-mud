...
class Attack:
    ...
    # async def start_attack(self, command: str, player: PlayerData):
    #     self.logger.debug("enter")

    #     room = self.world_service.rooms.rooms[player.room.id]
    ...
    #             for p in self.world_service.rooms.rooms[player.room.id].players:
    #                 if p.name == player.selected_character.name:
    #                     await InfoEvent(f"You begin to attack {current_monster.name}!").send(p.websocket)
    #                     p.in_combat = current_monster
    #                 else:
    #                     await InfoEvent(f"{player.selected_character.name} begins to attack {current_monster.name}!").send(p.websocket)

    #             # if you die and go to the crypt then your room id will change..
    #             while current_monster.hitpoints > 0 and player.room.id == self.world_service.rooms.rooms[player.room.id].id:
    ...
    #                     else:
    #                         if damage == 0:
    #                             response = f"{player.selected_character.name} swings wildly and misses!"
    #                         else:
    #                             if num_swings == 1:
    #                                 response = f"{player.selected_character.name} {player.weapon.plural_verb} {current_monster.name} with their {player.weapon.name.lower()} for {str(damage)} damage!"
    #                             else:
    #                                 response = f"{player.selected_character.name} {player.weapon.plural_verb} {current_monster.name} {num_swings} times with their {player.weapon.name.lower()} for {str(damage)} damage!"
    #                             await InfoEvent(response).send(p.websocket)
    ...
    #                     # set monster as dead
    #                     await current_monster.kill(self.world_service.rooms.rooms[player.room.id])
    ...
    #     self.world_service.rooms.rooms[player.room.id].monsters = room_monsters
    #     self.logger.debug("exit")
    #     return player

    async def execute(self, command: str, player: PlayerData):
        self.logger.debug("enter")
        # item = asyncio.create_task(self.start_attack(command, player, self.world_service))
